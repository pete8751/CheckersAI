...
def utility(state: CheckersGame) -> float:
    ...
    #     win = 0
    #
    # #### SOS ----- the static evaluation function must be turn agnostic (before I had it only for black...)
    # if state.get_turn() == True:
    #     us = state.black_survivors
    #     them = state.red_survivors
    # else:
    #     us = state.red_survivors
    #     them = state.black_survivors
    #
    # diff = (len(us) - len(them))
    # return diff + win + sum([state.stones[i].position[1] for i in us])/len(us)
    ...
    #### SOS ----- WHAT I HAD WRITTEN HERE IS INCORRECT
    black = state.black_survivors
    red = state.red_survivors

    diff = (len(black) - len(red))
    return diff + win + sum([state.stones[i].position[1] for i in black])/len(black)
